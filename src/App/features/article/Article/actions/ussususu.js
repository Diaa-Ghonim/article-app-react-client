...
export const addArticle = (article) => async (dispatch) => {
  ...
  try {
    ...
    if (error.response) {
      ...
      dispatch({
        type: ERROR_HAPPEN,
        payload: 'Something wrong happen please try again !!',
      });
    } else {
      dispatch({
        type: ERROR_HAPPEN,
...
export const removeLikeArticle = (articleID) => async (dispatch) => {
  try {
    const response = await axios.post('/api/articles/remove-like', {
      articleID,
    });
    const data = response.data;
    console.log(response);
...
export const removeDislikeArticle = (articleID) => async (dispatch) => {
  try {
    const response = await axios.post('/api/articles/remove-dislike', {
      articleID,
    });
    const data = response.data;
...
export const removeSaveArticle = (articleID) => async (dispatch) => {
  try {
    const response = await axios.post('/api/articles/remove-save', {
      articleID,
    });
    const data = response.data;
